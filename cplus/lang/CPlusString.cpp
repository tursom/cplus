...
namespace cplus {
	namespace lang {
		CPlusString::CPlusString(const ByteArray &str)
				: bufferSize(str.getSize() + 1) {
			this->str = (char *) malloc(bufferSize);
			memcpy(this->str, str.getBuffer(), str.getSize());
			this->str[str.getSize()] = 0;
		}
...
//			if (str < rhs.str) return true;
		...
		int16_t CPlusString::getCount() const {
			return count;
		}
		
		int16_t &CPlusString::getCount() {
			return count;
		}
		
		void CPlusString::lock() {
			mutex.lock();
		}
		
		void CPlusString::unlock() {
			mutex.unlock();
		}
		
		CPlusString::CPlusString(const char *str, size_t len) : bufferSize(len + 1) {
			this->str = (char *) malloc(bufferSize);
