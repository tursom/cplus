...
namespace cplus {
	namespace lang {
		...
		public:
			...
			std::shared_ptr<CPlusString> insert(CPlusString &value) {
//				std::cout << value.getStr() << std::endl;
			...
			std::shared_ptr<CPlusString> get(CPlusString &value) {
...
//					std::cout << "Set: not find key, inserting" << std::endl;
			...
			void forEach(const std::function<void(std::shared_ptr<CPlusString>)> &func) const {
				if (root != nullptr)
		...
		private:
			...
			public:
				...
				std::shared_ptr<CPlusString> get() const { return value; }
				
				TreeNode *insert(CPlusString &value) {
					return insert(this, value);
				}
				
				std::shared_ptr<CPlusString> find(const CPlusString &value) {
...
//						std::cout << "Set: static: state: " << state << std::endl;
			...
			private:
				...
				std::shared_ptr<CPlusString> value = nullptr;
			};
...
//		StringSet *stringSet = new StringSet();
		...
		String::String(const std::string &str) {
			value = std::make_shared<CPlusString>(CPlusString(str));
		}
		...
		String::String(const ByteArray &buffer) {
			value = std::make_shared<CPlusString>(CPlusString(buffer));
		}
